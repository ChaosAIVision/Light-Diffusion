...
WEIGHT_LPIPS_LOSS = os.getenv('WEIGHT_LPIPS_LOSS')
...
class LPIPS(nn.Module):
    def __init__(self, net='vgg', version='0.1', use_dropout=True):
        ...
        self.load_state_dict(torch.load(os.getenv('WEIGHT_LPIPS_LOSS'), map_location=device), strict=False)
        ########################
...
def masked_mse_loss(pred, target, mask):
    """
    Compute masked MSE normalized by mask area.

    Args:
        pred (torch.Tensor): Predicted tensor.
        target (torch.Tensor): Target tensor.
        mask (torch.Tensor): Mask tensor (broadcastable to pred/target).

    Returns:
        torch.Tensor: Per-sample loss with shape (batch,).
    """
    diff_sq = (pred - target) ** 2
    masked_diff = diff_sq * mask
    loss = masked_diff.sum(dim=(1, 2, 3)) / mask.sum(dim=(1, 2, 3)).clamp(min=1)
    return loss
